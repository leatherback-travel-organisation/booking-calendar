-- A live Cove invitation is already an approved employee record. Systems can
-- assign ownership and prepare grants before first sign-in; authentication and
-- enforcement still require Google to bind a verified identity to that record.

create or replace function is_approved_cove_employee(p_user_id uuid)
returns boolean
language sql
security invoker
stable
    ...
    where u.id = p_user_id
      ...
      and (
        ...
        or exists (
          ...
          where lower(invitation.email) = lower(u.email)
...
comment on function is_approved_cove_employee(uuid)
is 'True for active employee records backed by either a verified identity or a live pending Cove invitation.';
...
is 'Auth-owned, idempotent application/User/Admin provisioning primitive. Active employees and live invitees may be provisioned before first sign-in.';
