-- Reassessed seed statuses (Nicola, 31 Aug 2026): every supplied Gardening
-- project is CURRENT. Stages now come straight from the source lists —
-- "Active: Yes"/"In progress" = Active work; "Active: No"/"Not started"/blank
-- = In Planning (demo-marked). Nothing is cancelled, complete, testing, or
-- archived, so the archive starts empty. Replaces the stages 052 invented.
-- Generated from src/lib/garden/seed-data.ts — regenerate, do not hand-edit.
-- Guard: only rows untouched since seeding (last_edited_by is null) are
-- replaced; acknowledgement/overlap rows cascade away with them.
