...
const TYPES = {
  ...
  rhime: {
    noun: "booking call",
    purpose: "we'll walk through the trip together, make sure it's the right match, and reserve your spot",
...
if (process.argv.includes("--emit-migration")) {
  const updates = rows
    .map(
      (row) =>
        `  ('${row.brandKey}', '${row.typeKey}', '${row.moment}',\n` +
        `   '${sqlEscape(row.subject)}',\n` +
        `   '${sqlEscape(row.bodyHtml)}')`,
    )
    .join(",\n");
  const migration =
    `-- Regenerate the type-voice guest templates from the seed generator\n` +
    `-- (Leatherback Writing Guide pass, Nicola 28 Aug). Only rows still\n` +
    `-- marked 'seed:type-voice' change; anything a person edited is theirs.\n` +
