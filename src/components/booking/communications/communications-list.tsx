...
function editorHref(moment: string, brandKey: string, typeKey?: string): string {
...
export function CommunicationsList({ groups, canEdit }: CommunicationsListProps) {
  return (
    <div className={styles.journey}>
      <p className={styles.intro}>
        The emails each brand sends a guest, in the order they arrive. Pick a call type to read or change
        that version of the message.
        {canEdit ? "" : " Reminder switches are set by Pod Leads and Senior BMs."}
      </p>
      {groups.map(({ summary, colorPrimary, callTypes, ...flags }) => {
        const brandKey = summary.brandKey;
        return (
          <section
            key={brandKey}
            className={styles.brandGroup}
            style={colorPrimary ? ({ "--tag": colorPrimary } as CSSProperties) : undefined}
          >
            <div className={styles.brandHead}>
              <span className={styles.brandDot} aria-hidden="true" />
              <h2 className={styles.brandTitle}>{summary.brandName}</h2>
              <span className={styles.brandSetting}>
                <span className={styles.brandSettingLabel}>Text-message reminders</span>
                <BrandSwitch
                  brandKey={brandKey}
              ...
              {summary.moments.map((cell) => {
                ...
                const reminder =
                  cell.moment === "reminder_24h"
                    ? { kind: "reminder_24h" as const, enabled: flags.reminder24hEnabled }
                    : cell.moment === "reminder_1h"
                      ? { kind: "reminder_1h" as const, enabled: flags.reminder1hEnabled }
                      : null;
                return (
                  <li key={cell.moment} className={styles.messageRow} data-off={reminder && !reminder.enabled ? "" : undefined}>
                    <div className={styles.messageText}>
                      <Link href={editorHref(cell.moment, brandKey)} className={styles.messageName}>
                        {meta.label}
                      </Link>
