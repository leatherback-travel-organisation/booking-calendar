...
export function RecruitmentWorkspaceView({ workspace }: { workspace: RecruitmentWorkspace }) {
  ...
  const visibleCandidates = useMemo(() => {
    ...
    return candidates.filter((candidate) => {
      ...
      return !needle || `${candidate.name} ${candidate.surname ?? ""} ${candidate.email ?? ""} ${candidate.location ?? ""} ${candidate.yearsOfExperience ?? ""} ${candidate.mostRecentRoleEmployer ?? ""} ${(candidate.keySkills ?? []).join(" ")} ${candidate.educationLevel ?? ""} ${candidate.referralSource ?? ""} ${candidate.roles.join(" ")} ${(candidate.tags ?? []).join(" ")}`.toLowerCase().includes(needle);
    });
      ...
      {selectedCandidate && <div className={styles.overlay} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) { setSelectedCandidate(null); setEditingInterviewNotes(false); } }}><aside className={styles.record} role="dialog" aria-modal="true" aria-labelledby="candidate-title">
        ...
        <div className={styles.recordGrid}>
          <div className={styles.recordOverview}>
            <section className={`${styles.recordCard} ${styles.personalCard}`}><h3>Personal details</h3><div className={styles.candidateSummary}><span className={styles.largeAvatar}>{initials(selectedCandidate.name)}</span><div><span>{selectedCandidate.status}</span><strong>{selectedCandidate.location || "Location not added"}</strong><small>Updated {dateLabel(selectedCandidate.updatedAt || selectedCandidate.createdAt)}</small></div></div><div className={styles.contactRow}>{selectedCandidate.email ? <><a href={`mailto:${selectedCandidate.email}`}>{selectedCandidate.email}</a><button type="button" onClick={() => copyEmail(selectedCandidate.email)} aria-label="Copy email"><Icon name="copy"/>Copy</button></> : <span>Email not added</span>}<span>{selectedCandidate.schedule.join(" · ") || "Schedule not added"}</span></div><dl className={`${styles.detailList} ${styles.personalDetailsList}`}><div><dt>Full name</dt><dd>{selectedCandidate.name}</dd></div><div><dt>Surname</dt><dd>{selectedCandidate.surname || "Not added"}</dd></div><div><dt>Location — city/region</dt><dd>{selectedCandidate.location || "Not added"}</dd></div><div><dt>Years of experience</dt><dd>{selectedCandidate.yearsOfExperience || "Not added"}</dd></div><div><dt>Most recent role / employer</dt><dd>{selectedCandidate.mostRecentRoleEmployer || "Not added"}</dd></div><div><dt>Key skills</dt><dd>{selectedCandidate.keySkills?.join(" · ") || "Not added"}</dd></div><div><dt>Education level</dt><dd>{selectedCandidate.educationLevel || "Not added"}</dd></div><div><dt>Referral source</dt><dd>{selectedCandidate.referralSource || "Not added"}</dd></div>{selectedCandidate.assignee && <div><dt>Assignee</dt><dd>{selectedCandidate.assignee}</dd></div>}{selectedCandidate.interviewer && <div><dt>Interviewer</dt><dd>{selectedCandidate.interviewer}</dd></div>}</dl></section>
